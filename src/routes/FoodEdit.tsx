...
function FoodEdit() {
    ...
    const { isPending, isError, data: foodItem, error} = useQuery<Food>({
        queryKey: ['foodItem'],
        queryFn: async () => {

            try {
                const response = await axiosInstance.get(
                    `/api/FoodItem/${params.id}`);
                ...
                //setServingList(response.data.servingSizes);
                setServingList(cloneDeep(response.data.servingSizes));
                return response.data;

            } catch(error) {
                return Promise.reject(new Error('Food item does not exist'));
            }
        },
        retry: false,
    });
    ...
    const updateMutation = useMutation({
        mutationFn: (foodData:{id:number, name:string, energy:number, carbs:number, fat:number, protein:number, servingSizes: ServingSize[]}) => {

            return axiosInstance.put(`/api/FoodItem/${foodData.id}`, foodData, {
    ...
    const deleteMutation = useMutation({
        mutationFn: () => {
            return axiosInstance.delete(`/api/FoodItem/${params.id}`);
        },
        onSuccess: () => {
            // Invalidate profiles
    ...
    function undoChanges()
    {
        if (foodItem !== undefined) {
            ...
            setEnergy(foodItem.energy);
            setCarbs(foodItem.carbs);
            setFat(foodItem.fat);
            setProtein(foodItem.protein);
            setServingList(cloneDeep(foodItem.servingSizes));
        }
    }
    ...
    function handleDeleteRows() {
        ...
        //console.log(JSON.stringify(newServingList));


        // selectedRows.forEach((val) => {
        //     newServingList.splice(val, 1);
        // });

        // setSelectedRows([]);

        // if (servingList !== undefined) {
        //     const test = servingList.filter(s => selectedRows.indexOf(s.id) !== -1);
        //
        //     console.log(JSON.stringify(test));
        // }


        //setServingList(servingList.filt)
                    ...
                    <Grid container spacing={{xs: 2, md: 3}} columns={{ xs:4, sm: 8, md: 12}} alignItems="center">
                        ...
                        <Grid xs={4}>
                            ...
                            <TableContainer component={Paper} sx={{maxHeight:300}}>
                                ...
                                {/*    <IconButton aria-label="add" size="medium" onClick={(e)=> onAddServing(e)}>*/}
                                {/*        <AddCircleIcon fontSize="inherit" />*/}
                                {/*    </IconButton>*/}
                                {/*    <IconButton aria-label="add" size="medium">*/}
                                {/*        <EditIcon fontSize="inherit" />*/}
